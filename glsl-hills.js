...
export function initGLSLHills(container, options = {}) {
  const { cameraZ = 125, planeSize = 256, speed = 0.5 } = options;
  ...
  // Reduced segment count from 256→128 = 4× fewer vertices (16K vs 65K)
  const segments = 128;
  ...
  class Plane {
        ...
        new THREE.RawShaderMaterial({
          ...
          fragmentShader: `
            ...
            void main(void) {
              ...
              vec3 color = vec3(0.0, 1.0, 0.25); /* Green tint matching brand */
    ...
    render(dt) {
      this.uniforms.time.value += dt * this.time;
    }
  }
  ...
  function renderLoop() {
    ...
    if (!running) return; // skip rendering when off-screen
  ...
  const observer = new IntersectionObserver(([entry]) => {
    ...
    if (running) clock.getDelta(); // reset clock delta to avoid large jump
